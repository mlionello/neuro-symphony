function run_h4_pca_composite_permutation_test(nPerm)
% RUN_H4_PCA_COMPOSITE_PERMUTATION_TEST(nPerm)
%
% Null distribution for the "all 6 buttons" (PCA composite)
% between-synchrony table.
%
% Writes: analysis/figures/h4_pca_composite_permutation_test_exp1-exp2.csv,
% same schema as h4_permutation_test_exp1-exp2.csv (minus button/
% event_type, constant here).
%
% Requires MATLAB Statistics Toolbox (R2019a+) and Finn Upham's
% ActivityAnalysisToolbox_2.1-master/.

if nargin < 1 || isempty(nPerm)
    nPerm = 999;
end
...
outCsv      = fullfile(repoRoot, 'analysis', 'figures', 'h4_pca_composite_permutation_test_exp1-exp2.csv');
...
for gi = 1:height(groupKeys)
    ...
    usableGroups(end+1) = struct('expid', groupKeys.expid{gi}, 'trackNumber', groupKeys.track_number(gi), ...
        'mats', mats, 'minLen', minLen, 'coherIds', {coherIds}, 'oppIds', {oppIds});
end

nGroupsUsed = numel(usableGroups);
fprintf('Usable (expid, track_number) groups: %d\n', nGroupsUsed);

%% ---- Pre-allocate: (1 observed + nPerm null draws) x nGroupsUsed x nEventTypes x nButtons x 2 pseudo-groups ----
nDraws = 1 + nPerm;
nRowsTotal = nDraws * nGroupsUsed * numel(EVENT_TYPES) * numel(BUTTON_LABELS) * 2;
results = cell(nRowsTotal, 9);
rowIdx = 0;
    ...
    for drawIdx = 0:nPerm
        ...
        for bIdx = 1:numel(BUTTON_LABELS)
            ...
            for eIdx = 1:numel(EVENT_TYPES)
                ...
                ACcoher = mean(coherFrames, 1)';
                [~, pCoher, ~, binsCoher] = simpleActivityTest(ACcoher, numel(pseudoCoher), WITHIN_NBINS);
                [Ccoher, feasCoher, nbCoher] = score_from_bins(pCoher, binsCoher);

                ACopp = mean(oppFrames, 1)';
                [~, pOpp, ~, binsOpp] = simpleActivityTest(ACopp, numel(pseudoOpp), WITHIN_NBINS);
                [Copp, feasOpp, nbOpp] = score_from_bins(pOpp, binsOpp);

                rowIdx = rowIdx + 1;
                results(rowIdx, :) = {expid, trackNumber, BUTTON_LABELS{bIdx}, eventType, drawIdx, ...
                    'Coher_role', Ccoher, pCoher, feasCoher && ~(nbCoher == 2)};
                rowIdx = rowIdx + 1;
                results(rowIdx, :) = {expid, trackNumber, BUTTON_LABELS{bIdx}, eventType, drawIdx, ...
                    'Opp_role', Copp, pOpp, feasOpp && ~(nbOpp == 2)};
            end
        end
    end
end
...
resultsTable = cell2table(results, 'VariableNames', ...
    {'expid', 'track_number', 'button', 'event_type', 'perm_idx', 'pseudo_condition', 'C', 'p', 'reliable'});
...
switch eventType
    ...
    otherwise
        error('run_h4_pca_composite_permutation_test:build_event_frames:UnknownEventType', ...
