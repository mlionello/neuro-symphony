...
This folder contains the web application used to collect behavioral data for the contextual information manipulation study.
...
3. Preliminary questionnaires (one-time): demographics + screening (language, psychiatric exclusion items), Gold-MSI, BFI-10.
4. Per-session questionnaire: WHO-5 at the beginning of each listening session.
5. Listening session (per symphony, split across movements): read a program note, listen to the movement while interacting continuously with 6 emotion/sensation buttons, then give post-movement ratings and feedback.
...
The study follows a 2×2 factorial design crossing affective coherence (description valence coherent vs opposite to the music) and narrative authenticity (reference vs injected narrative source), assigning all four conditions across the four movements within an experiment, with balanced/randomized pairings.
...
neurosymphony/
  ...
  preprocessing/                # analysis / preprocessing scripts (not required to run the website)
  analysis/                     # exported data/plots (not required to run the website)
```
...
- A web server: Apache2 (closest to production), or the PHP built-in dev server for quick testing.

### 2) Run with PHP's built-in server (fastest)
From the repository root:

```bash
php -S localhost:8000 -t neurosymphony/web
```
...
```bash
sudo chgrp -R www-data neurosymphony/web/userdata
sudo chmod -R 770 neurosymphony/web/userdata
```

## Required configuration

### Global secret key (encryption)
The site encrypts sensitive values (e.g., email / user ID) using AES-256-CBC, with a user-specific key derived from the username plus a server-side global key.

In `web/security/endecrypt.php` there is a `key_path` pointing to a server-local file (e.g., `/var/secure/keys/neurosymphony.key`). Create that file and restrict permissions:

```bash
sudo mkdir -p /var/secure/keys
openssl rand -hex 32 | sudo tee /var/secure/keys/neurosymphony.key >/dev/null
sudo chmod 600 /var/secure/keys/neurosymphony.key
```
...
In production the app uses a CSV credential store (hashed usernames + password hashes, plus encrypted fields). Do not commit this file.

Create an empty file to start:

```bash
touch neurosymphony/web/users.csv
chmod 600 neurosymphony/web/users.csv
```
...
Downstream preprocessing discards timestamps, aligns participants, smooths, and downsamples.
...
A hardened production setup includes:
